...
macro_rules! define_recorder_dispatch {
    ($this:ident, $input:ident, $object:ident; $( $variant:ident $wire_pattern:tt $record_pattern:tt => $tag:literal; record $record:block wire $wire:block parse $parse:block )*) => {
        fn record(&mut self, event: RunEvent) {
            let $this = self;
            let $input = event;
            match &$input { $(RunEvent::$variant $record_pattern => $record,)* }
...
impl SessionRecorder {
    ...
    crate::session::catalog::run_event_seats!(define_recorder_dispatch, recorder, input, object);
}
