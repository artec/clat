...
impl App {
    ...
    /// /skill 列表弹窗（SC-2）：每技能一行（名称 · 来源层 · 约束 — 描述）
    /// + 发现诊断节。一次性快照（无 `r` 刷新），骨架同 /help 与 /context。
    pub(super) fn draw_skills_dialog(&mut self, frame: &mut Frame) {
        let area = frame.area();
        let inner_width = popup_inner_width(84, area);
        let Some(view) = self.skills_view.as_ref() else {
            return;
        };
