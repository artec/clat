...
/// writer 队列条目。`pub(crate)`：mcp_client 的 dispatcher 经
...
impl StdioSession {
    ...
    /// 安装服务端请求的投递通道（mcp_client 的 dispatcher 在 connect
...
/// HTTP 会话的响应端点零件（服务端请求的回写通道，mcp_client 的
/// Responder 组装用）。
...
impl HttpSession {
    ...
    /// 响应端点零件（mcp_client 的 Responder）：独立 POST 回写服务端
