...
CLAT 是一个稳如磐石的 Agent 底座：快速、本地优先、开源，以 Rust
单二进制交付。项目从一条简单的规矩出发：先造我们自己每天真想用的
智能体，在真实仓库上 dogfood，再把这些需求泛化为可复用的开源能力。
...
- `clat exec` 面向脚本与 CI 的无头单次运行——同一套权限模型；
  `--json` 以带版本号的 NDJSON 流式输出全部运行事件，供编辑器与
  管道消费
- `clat serve` 在 127.0.0.1 上提供本地 HTTP+SSE API——token 门禁、
  事件流式输出，是即将到来的浏览器客户端的地基
