...
- **DSH alignment work must start from the upgrade skill** (`.agents/skills/dsh-session-upgrade/SKILL.md`, Agent Skills open-standard location — Claude Code users: symlink `.claude/skills` → `.agents/skills` or read the file directly). It contains the phased playbook, the migration invariants, and the known dead ends from the 2026-09 v0→v2 campaign; skipping it re-pays tuition the project already paid.
