...
- **Local gates mirror CI (2026-09-07 decision, archive `docs/research/ci-parity.md`).** Run `scripts/gates.sh` before handoff — it mirrors the ci.yml Linux job step by step, including the `-- --ignored` gated face and the adapter npm legs; add `--stress N` for timing/concurrency-sensitive changes; add `scripts/ci-box.sh` (Linux container, default 2 CPUs) for platform-semantics changes (file locks, process trees, TCP timing, low-core contention). The toolchain is pinned by `rust-toolchain.toml`; upgrading it is a deliberate same-batch change. When ci.yml steps change, update gates.sh/ci-box.sh in the same change — divergence between them is a bug.
