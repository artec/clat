...
| Permission modes (DSH presets, adapted) | done — three user-switchable modes (`Read Only` / `Project Write` default / `Full Access`) over the `ToolEffect` table; a shared mode cell makes switches effective at the next check; the permission dialog offers exactly the wider modes that would allow the pending call (`w`/`f` escalation keys), `/perm` (alias `/permission`) is the cold-switch/downgrade path with a Full Access confirm step; `clat exec` stays on `SafeByDefault` (zero behavior change); no shell-command classification and no mode-level deny — refusals stay human. The mode persists per project in a root-level control-plane JSON file (`permission_modes.json` — deliberately outside the version-locked clat.db schema; corrupt/missing files degrade to the default). See permissions.md |
